#include "gravity.hpp"

Eigen::Vector3d dso::point_mass_acceleration(const Eigen::Vector3d &r,
                                             const Eigen::Vector3d &rcb,
                                             double GMcb) noexcept {

  const double Rcb3 = rcb.squaredNorm() * rcb.norm();
  const auto s = r - rcb;
  const double S3 = s.squaredNorm() * s.norm();

  return (s / S3 + rcb / Rcb3) * (-GMcb);
}

Eigen::Vector3d
dso::point_mass_acceleration(const Eigen::Vector3d &r,
                             const Eigen::Vector3d &rcb, double GMcb,
                             Eigen::Matrix<double, 3, 3> &J) noexcept {

  const double Rcb3 = rcb.squaredNorm() * rcb.norm();
  const auto s = r - rcb;
  const double S3 = s.squaredNorm() * s.norm();
  const double S5 = S3 * s.squaredNorm();

  /* jacobian */
  J = (-GMcb) * (Eigen::Matrix<double, 3, 3>::Identity() / S3 -
                 3e0 * s * s.transpose() / S5);

  /* acceleration vector */
  return (s / S3 + rcb / Rcb3) * (-GMcb);
}
