...
int sh2gradient_cunningham_impl(
  ...
  for (int m = order; m >= 2; --m) {
    ...
    const double *fsqrt5 = Cf.grad_scale.data() + degree;
#else
    const double *fsqrt3 = psq.sqnp3.data() + degree; /* square root factors */
    const double *fsqrt5 = psq.sqnp5.data() + degree;
#endif
  ...
  const double *d2wm0 = Cf.d2_m0_wm0.data() + row_idx0;
  const double *d2wp1 = Cf.d2_m0_wp1.data() + row_idx0;
  const double *d2wp2 = Cf.d2_m0_wp2.data() + row_idx0;
