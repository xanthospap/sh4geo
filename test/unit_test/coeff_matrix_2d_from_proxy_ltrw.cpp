...
int main() {
  CoeffMatrix2D<MatrixStorageType::LwTriangularRowWise> mat1(10, 10);
  ...
  for (int i = 0; i < mat1.rows(); i++) {
    ...
    CoeffMatrix2D<MatrixStorageType::LwTriangularRowWise> B(expr);
    ...
    CoeffMatrix2D<MatrixStorageType::LwTriangularRowWise> C(2.0 * mat1.reduced_view(7,7));
    assert(C.rows()==7);
    assert(C.cols() == 7);
