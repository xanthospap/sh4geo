...
int main() {
  CoeffMatrix2D<MatrixStorageType::LwTriangularColWise> mat1(5);
  CoeffMatrix2D<MatrixStorageType::LwTriangularColWise> mat2(5);
  ...
  mat3 += 1e0 * mat1.reduced_view(3) + 2e0 * mat2.reduced_view(3);
  assert(mat3.rows() == 3);
  assert(mat3.cols() == 3);
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j <= i; j++) {
