...
enum class ReductionAssignmentOperator { Equal,
  EqAdd };
...
inline double& op(double& lhs, double rhs) noexcept
{
  ...
  /* make life easier, in C++20 we can emmit a compile-time diagnostic 
   * see CoeffMatrix2D::make_storage_impl for details/usage
   */
  template <class T>
...
template <class T>
struct _is_expr<T, std::void_t<decltype(std::declval<const T&>().rows()), decltype(std::declval<const T&>().cols()), decltype(std::declval<const T&>()(0, 0))>>
    : std::true_type { };

/* utility class expose for _is_expr */
template <class T>
static constexpr bool _is_expr_v = _is_expr<std::remove_cv_t<std::remove_reference_t<T>>>::value;
...
class CoeffMatrix2D {
   ...
   */
  /* factory for constructing a Storage<S> */
  static Storage make_storage_impl(int rows, int cols)
  {
    if constexpr (Storage::isSquare) {
#if __cplusplus >= 202002L
      static_assert(c20xSquareConstructible<Storage>, "Square storage must be constructible as Storage(int)");
#endif
      (void)cols;
      return Storage(rows);
    } else {
#if __cplusplus >= 202002L
      static_assert(c20xRectConstructible<Storage>, "Rect storage must be constructible as Storage(int,int)");
#endif
  ...
  void cresize_impl(int rows, int cols)
  {
    ...
    if (m_data) {
      ...
      for (int s = 0;
          s < std::min(new_storage.num_slices(), m_storage.num_slices()); s++) {
        ...
        std::memcpy(ptrg, psrc,
            sizeof(double) * std::min(num_doubles_src, num_doubles_trg));
      }
      delete[] m_data;
    }
...
public:
  ...
  class _ScaledProxy {
    ...
    Expr expr;
    double fac;
    _ScaledProxy(Expr e, double d) noexcept
        : expr(std::move(e))
        , fac(d) { };

public:
    static constexpr const int hasContiguousMem = std::remove_reference_t<Expr>::hasContiguousMem;
...
public:
    static constexpr const int hasContiguousMem = std::remove_reference_t<L>::hasContiguousMem && std::remove_reference_t<R>::hasContiguousMem;
  ...
  double& operator()(int i, int j) noexcept
  {
    ...
    assert(m_storage.index(i, j) >= 0 && m_storage.index(i, j) < (int)m_storage.num_elements());
#endif
